...
export class PersonaListComponent implements OnInit {
  ...
  private metadata : PersonaMetadata;
  private personaDomain : PersonaDomain = new PersonaDomain();
  ...
  ngOnInit(): void {
  	this.fieldList = this.personaDomain.getSearchListOfFields();
    this.metadata = this.personaDomain.getPersonaMetadata();
	this.getPersonaList(null);
  ...
  advancedSearch() {
   if (this.queryParamList !== null && this.queryParamList.length > 0) {
     if (this.queryParamList.length === 1 && this.queryParamList[0][0].length === 0) {
       ...
       this.getPersonaList(this.queryParamList);
     }
   	 } else {
       this.getPersonaList(null);
     }
   ...
   onPersonaDelete(persona : Persona) {
    let queryParam : Array<[string, string]> = new Array<[string, string]>();
    queryParam.push(['tipoDocumento', persona.personaPK.tipoDocumento.toString()]);
    queryParam.push(['numDocumento', persona.personaPK.numDocumento.toString()]);
    this.deletePersona(queryParam);
  }
  ...
  onSelectedPersona(persona: Persona){
   ...
   getPersonaList(queryParamList: [string, string][]) {
    ...
    this.sub = this.route
      ...
      .subscribe(params => {
        this.genericClientService.get(uriService)
          .subscribe(result => {this.personaList = result; this._spinnerService.hide(); },
                       err  => this.cathException('error_' + err)
          );
      });
   }
   
   deletePersona(queryParam: Array<[string, string]>) {
   	this._spinnerService.show();
    let idResult: any = 0;
    let serviceProvider : string = 'Persona';
    let pathParams : Array<[number, string]> = null;
