...
import { AforoMetadata } from '../../core/domain/aforo.domain';
import { AforoDomain } from '../../core/domain/aforo.domain';
...
@Component({
  selector: 'aforo-list',
  templateUrl: './aforo-list.component.html',
  providers:  [UriProvider, GenericClientService]
})
export class AforoListComponent implements OnInit {

  private sub: any;
  private aforoList : Aforo[];
  private queryParamList: [string, string][] = [['','']];

  private metadata : AforoMetadata;
  private aforoDomain : AforoDomain = new AforoDomain();
  private fieldList: Field[];
  private modalDeleteMessage : ConfirmationModalOptions;
  private openGenericSearch : boolean = false;
  private localAlert : AlertObject = new AlertObject(null, null, null, 0);
	
  private selectedAforo: Aforo;
  constructor(
  ...
  ngOnInit(): void {
  	this.fieldList = this.aforoDomain.getSearchListOfFields();
    this.metadata = this.aforoDomain.getAforoMetadata();
	this.getAforoList(null);
  ...
  advancedSearch() {
   if (this.queryParamList !== null && this.queryParamList.length > 0) {
     if (this.queryParamList.length === 1 && this.queryParamList[0][0].length === 0) {
       ...
       this.getAforoList(this.queryParamList);
     }
   	 } else {
       this.getAforoList(null);
     }
   ...
   onAforoDelete(aforo : Aforo) {
    let queryParam : Array<[string, string]> = new Array<[string, string]>();
    queryParam.push(['idAforo', aforo.aforoPK.idAforo.toString()]);
    queryParam.push(['idPotrero', aforo.aforoPK.idPotrero.toString()]);
    queryParam.push(['idHacienda', aforo.aforoPK.idHacienda.toString()]);
    this.deleteAforo(queryParam);
  }
  
  onSelectedAforo(aforo: Aforo){
    this.selectedAforo = aforo;
  }
   
   getAforoList(queryParamList: [string, string][]) {
   	this._spinnerService.show();
    let serviceProvider : string = 'Aforo';
    ...
    this.sub = this.route
      ...
      .subscribe(params => {
        this.genericClientService.get(uriService)
          .subscribe(result => {this.aforoList = result; this._spinnerService.hide(); },
                       err  => this.cathException('error_' + err)
          );
      });
   }
   
   deleteAforo(queryParam: Array<[string, string]>) {
   	this._spinnerService.show();
    let idResult: any = 0;
    let serviceProvider : string = 'Aforo';
    let pathParams : Array<[number, string]> = null;
