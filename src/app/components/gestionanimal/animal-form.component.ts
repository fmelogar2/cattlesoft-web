...
export class AnimalFormComponent implements AfterViewChecked {
  @Input() animal : Animal;
  @ViewChild('animalForm') currentForm: NgForm;
  private sub: any;
  private animalDomain = new AnimalDomain();
  private isNewAnimal : boolean = false;
  private metadata : AnimalMetadata = new AnimalDomain().getAnimalMetadata();
  private formErrors : any = this.animalDomain.getFormErrors();
  private validationMessages : any = this.animalDomain.getValidationMessages();
  @Input()padre: any;
  ...
  formChanged() {
    if (this.currentForm === this.animalForm) { return; }
    this.animalForm = this.currentForm;
    if (this.animalForm) {
  ...
  onSubmit() {
    ...
    if (this.isNewAnimal) {
      this.post(this.animal);
    } else {
      this.put(this.animal);
    }
  ...
  onValueChanged(data?: any) {
    this.modalMessage.disabled = !this.animalForm.form.valid;
    if (!this.animalForm) { return; }
    const form = this.animalForm.form;
