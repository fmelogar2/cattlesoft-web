...
export class RazaListComponent implements OnInit {
  ...
  ngOnInit(): void {
  	this.fieldList = this.razaDomain.getSearchListOfFields();
    this.metadata = this.razaDomain.getRazaMetadata();
	this.getRazaList(null);
  ...
  advancedSearch() {
   if (this.queryParamList !== null && this.queryParamList.length > 0) {
     if (this.queryParamList.length === 1 && this.queryParamList[0][0].length === 0) {
       ...
       this.getRazaList(this.queryParamList);
     }
   	 } else {
       this.getRazaList(null);
     }
   ...
   onRazaDelete(raza : Raza) {
    let queryParam : Array<[string, string]> = new Array<[string, string]>();
    queryParam.push(['codigoRaza', raza.razaPK.codigoRaza.toString()]);
    queryParam.push(['idHacienda', raza.razaPK.idHacienda.toString()]);
    this.deleteRaza(queryParam);
  }
  
  onSelectedRaza(raza: Raza){
    this.selectedRaza = raza;
  }
   
   getRazaList(queryParamList: [string, string][]) {
    ...
    this.sub = this.route
      ...
      .subscribe(params => {
        this.genericClientService.get(uriService)
          .subscribe(result => {this.razaList = result; this._spinnerService.hide(); },
                       err  => this.cathException('error_' + err)
          );
      });
   }
   
   deleteRaza(queryParam: Array<[string, string]>) {
   	this._spinnerService.show();
    let idResult: any = 0;
    let serviceProvider : string = 'Raza';
    let pathParams : Array<[number, string]> = null;
