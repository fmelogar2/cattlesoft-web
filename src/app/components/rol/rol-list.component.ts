...
export class RolListComponent implements OnInit {
  ...
  ngOnInit(): void {
  	this.fieldList = this.rolDomain.getSearchListOfFields();
    this.metadata = this.rolDomain.getRolMetadata();
	this.getRolList(null);
  ...
  advancedSearch() {
   if (this.queryParamList !== null && this.queryParamList.length > 0) {
     if (this.queryParamList.length === 1 && this.queryParamList[0][0].length === 0) {
       ...
       this.getRolList(this.queryParamList);
     }
   	 } else {
       this.getRolList(null);
     }
   ...
   onRolDelete(rol : Rol) {
    ...
    queryParam.push(['idRol', rol.idRol.toString()]);
    this.deleteRol(queryParam);
  }
  
  onSelectedRol(rol: Rol){
    this.selectedRol = rol;
  }
   
   getRolList(queryParamList: [string, string][]) {
    ...
    this.sub = this.route
      ...
      .subscribe(params => {
        this.genericClientService.get(uriService)
          .subscribe(result => {this.rolList = result; this._spinnerService.hide(); },
                       err  => this.cathException('error_' + err)
          );
      });
   }
   
   deleteRol(queryParam: Array<[string, string]>) {
   	this._spinnerService.show();
    let idResult: any = 0;
    let serviceProvider : string = 'Rol';
    let pathParams : Array<[number, string]> = null;
