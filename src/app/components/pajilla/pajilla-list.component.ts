...
import { Pajilla } from '../../core/domain/pajilla.domain';
import { PajillaMetadata } from '../../core/domain/pajilla.domain';
import { PajillaDomain } from '../../core/domain/pajilla.domain';
...
@Component({
  selector: 'pajilla-list',
  templateUrl: './pajilla-list.component.html',
  providers:  [UriProvider, GenericClientService]
})
export class PajillaListComponent implements OnInit {

  private sub: any;
  private pajillaList : Pajilla[];
  private queryParamList: [string, string][] = [['','']];

  private metadata : PajillaMetadata;
  private pajillaDomain : PajillaDomain = new PajillaDomain();
  private fieldList: Field[];
  private modalDeleteMessage : ConfirmationModalOptions;
  private openGenericSearch : boolean = false;
  private localAlert : AlertObject = new AlertObject(null, null, null, 0);
	
  private selectedPajilla: Pajilla;
  constructor(
  ...
  ngOnInit(): void {
  	this.fieldList = this.pajillaDomain.getSearchListOfFields();
    this.metadata = this.pajillaDomain.getPajillaMetadata();
	this.getPajillaList(null);
  ...
  advancedSearch() {
   if (this.queryParamList !== null && this.queryParamList.length > 0) {
     if (this.queryParamList.length === 1 && this.queryParamList[0][0].length === 0) {
       ...
       this.getPajillaList(this.queryParamList);
     }
   	 } else {
       this.getPajillaList(null);
     }
   ...
   onPajillaDelete(pajilla : Pajilla) {
    let queryParam : Array<[string, string]> = new Array<[string, string]>();
    queryParam.push(['idPajilla', pajilla.idPajilla.toString()]);
    this.deletePajilla(queryParam);
  }
  
  onSelectedPajilla(pajilla: Pajilla){
    this.selectedPajilla = pajilla;
  }
   
   getPajillaList(queryParamList: [string, string][]) {
   	this._spinnerService.show();
    let serviceProvider : string = 'Pajilla';
    ...
    this.sub = this.route
      ...
      .subscribe(params => {
        this.genericClientService.get(uriService)
          .subscribe(result => {this.pajillaList = result; this._spinnerService.hide(); },
                       err  => this.cathException('error_' + err)
          );
      });
   }
   
   deletePajilla(queryParam: Array<[string, string]>) {
   	this._spinnerService.show();
    let idResult: any = 0;
    let serviceProvider : string = 'Pajilla';
    let pathParams : Array<[number, string]> = null;
