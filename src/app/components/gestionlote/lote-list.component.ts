...
export class LoteListComponent implements OnInit {
  ...
  ngOnInit(): void {
  	this.fieldList = this.loteDomain.getSearchListOfFields();
    this.metadata = this.loteDomain.getLoteMetadata();
	this.getLoteList(null);
  ...
  advancedSearch() {
   if (this.queryParamList !== null && this.queryParamList.length > 0) {
     if (this.queryParamList.length === 1 && this.queryParamList[0][0].length === 0) {
       ...
       this.getLoteList(this.queryParamList);
     }
   	 } else {
       this.getLoteList(null);
     }
   ...
   onLoteDelete(lote : Lote) {
    let queryParam : Array<[string, string]> = new Array<[string, string]>();
    queryParam.push(['idLote', lote.lotePK.idLote.toString()]);
    queryParam.push(['idHacienda', lote.lotePK.idHacienda.toString()]);
    this.deleteLote(queryParam);
  }
  
  onSelectedLote(lote: Lote){
    this.selectedLote = lote;
  }
   
   getLoteList(queryParamList: [string, string][]) {
    ...
    this.sub = this.route
      ...
      .subscribe(params => {
        this.genericClientService.get(uriService)
          .subscribe(result => {this.loteList = result; this._spinnerService.hide(); },
                       err  => this.cathException('error_' + err)
          );
      });
   }
   
   deleteLote(queryParam: Array<[string, string]>) {
   	this._spinnerService.show();
    let idResult: any = 0;
    let serviceProvider : string = 'Lote';
    let pathParams : Array<[number, string]> = null;
