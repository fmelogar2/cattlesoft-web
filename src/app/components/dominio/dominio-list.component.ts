...
export class DominioListComponent implements OnInit {
  ...
  private sub: any;
  private dominioList : Dominio[];
  private queryParamList: [string, string][] = [['','']];

  private metadata : DominioMetadata;
  private dominioDomain : DominioDomain = new DominioDomain();
  private fieldList: Field[];
  private modalDeleteMessage : ConfirmationModalOptions;
  private openGenericSearch : boolean = false;
  private localAlert : AlertObject = new AlertObject(null, null, null, 0);
	
  private selectedDominio: Dominio;
  constructor(
  ...
  ngOnInit(): void {
  	this.fieldList = this.dominioDomain.getSearchListOfFields();
    this.metadata = this.dominioDomain.getDominioMetadata();
	this.getDominioList(null);
  ...
  advancedSearch() {
   if (this.queryParamList !== null && this.queryParamList.length > 0) {
     if (this.queryParamList.length === 1 && this.queryParamList[0][0].length === 0) {
       ...
       this.getDominioList(this.queryParamList);
     }
   	 } else {
       this.getDominioList(null);
     }
   ...
   onDominioDelete(dominio : Dominio) {
    let queryParam : Array<[string, string]> = new Array<[string, string]>();
    queryParam.push(['dominio', dominio.dominioPK.dominio.toString()]);
    queryParam.push(['valor', dominio.dominioPK.valor.toString()]);
    this.deleteDominio(queryParam);
  }
  
  onSelectedDominio(dominio: Dominio){
    this.selectedDominio = dominio;
  }
   
   getDominioList(queryParamList: [string, string][]) {
    ...
    this.sub = this.route
      ...
      .subscribe(params => {
        this.genericClientService.get(uriService)
          .subscribe(result => {this.dominioList = result; this._spinnerService.hide(); },
                       err  => this.cathException('error_' + err)
          );
      });
   }
   
   deleteDominio(queryParam: Array<[string, string]>) {
   	this._spinnerService.show();
    let idResult: any = 0;
    let serviceProvider : string = 'Dominio';
    let pathParams : Array<[number, string]> = null;
