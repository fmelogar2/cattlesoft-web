...
import { MedicamentoMetadata } from '../../core/domain/medicamento.domain';
import { MedicamentoDomain } from '../../core/domain/medicamento.domain';
...
export class MedicamentoListComponent implements OnInit {
  ...
  private sub: any;
  private medicamentoList : Medicamento[];
  private queryParamList: [string, string][] = [['','']];

  private metadata : MedicamentoMetadata;
  private medicamentoDomain : MedicamentoDomain = new MedicamentoDomain();
  private fieldList: Field[];
  private modalDeleteMessage : ConfirmationModalOptions;
  private openGenericSearch : boolean = false;
  private localAlert : AlertObject = new AlertObject(null, null, null, 0);
	
  private selectedMedicamento: Medicamento;
  constructor(
  ...
  ngOnInit(): void {
  	this.fieldList = this.medicamentoDomain.getSearchListOfFields();
    this.metadata = this.medicamentoDomain.getMedicamentoMetadata();
	this.getMedicamentoList(null);
  ...
  advancedSearch() {
   if (this.queryParamList !== null && this.queryParamList.length > 0) {
     if (this.queryParamList.length === 1 && this.queryParamList[0][0].length === 0) {
       ...
       this.getMedicamentoList(this.queryParamList);
     }
   	 } else {
       this.getMedicamentoList(null);
     }
   ...
   onMedicamentoDelete(medicamento : Medicamento) {
    let queryParam : Array<[string, string]> = new Array<[string, string]>();
    queryParam.push(['idMedicamento', medicamento.idMedicamento.toString()]);
    this.deleteMedicamento(queryParam);
  }
  ...
  onSelectedMedicamento(medicamento: Medicamento){
   ...
   getMedicamentoList(queryParamList: [string, string][]) {
   	this._spinnerService.show();
    let serviceProvider : string = 'Medicamento';
    ...
    this.sub = this.route
      ...
      .subscribe(params => {
        this.genericClientService.get(uriService)
          .subscribe(result => {this.medicamentoList = result; this._spinnerService.hide(); },
                       err  => this.cathException('error_' + err)
          );
      });
   }
   
   deleteMedicamento(queryParam: Array<[string, string]>) {
   	this._spinnerService.show();
    let idResult: any = 0;
    let serviceProvider : string = 'Medicamento';
    let pathParams : Array<[number, string]> = null;
  ...
  getConfirmationResponse($event : [boolean, any]) {
    if ($event[0]) {
      ...
      this.onMedicamentoDelete($event[1]);
    }
  }
