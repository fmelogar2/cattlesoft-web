...
@Component({
  selector: 'chequeo-mastitis-list',
  templateUrl: './chequeo-mastitis-list.component.html',
  providers:  [UriProvider, GenericClientService]
})
export class ChequeoMastitisListComponent implements OnInit {

  private sub: any;
  private chequeoMastitisList : ChequeoMastitis[];
  private queryParamList: [string, string][] = [['','']];

  private metadata : ChequeoMastitisMetadata;
  private chequeoMastitisDomain : ChequeoMastitisDomain = new ChequeoMastitisDomain();
  private fieldList: Field[];
  private modalDeleteMessage : ConfirmationModalOptions;
  private openGenericSearch : boolean = false;
  private localAlert : AlertObject = new AlertObject(null, null, null, 0);
	
  private selectedChequeoMastitis: ChequeoMastitis;
  constructor(
  ...
  ngOnInit(): void {
  	this.fieldList = this.chequeoMastitisDomain.getSearchListOfFields();
    this.metadata = this.chequeoMastitisDomain.getChequeoMastitisMetadata();
	this.getChequeoMastitisList(null);
  ...
  advancedSearch() {
   if (this.queryParamList !== null && this.queryParamList.length > 0) {
     if (this.queryParamList.length === 1 && this.queryParamList[0][0].length === 0) {
       ...
       this.getChequeoMastitisList(this.queryParamList);
     }
   	 } else {
       this.getChequeoMastitisList(null);
     }
   ...
   onChequeoMastitisDelete(chequeoMastitis : ChequeoMastitis) {
    let queryParam : Array<[string, string]> = new Array<[string, string]>();
    queryParam.push(['idAnimal', chequeoMastitis.chequeoMastitisPK.idAnimal.toString()]);
    queryParam.push(['fechaChequeo', chequeoMastitis.chequeoMastitisPK.fechaChequeo.toString()]);
    this.deleteChequeoMastitis(queryParam);
  }
  
  onSelectedChequeoMastitis(chequeoMastitis: ChequeoMastitis){
    this.selectedChequeoMastitis = chequeoMastitis;
  }
   
   getChequeoMastitisList(queryParamList: [string, string][]) {
    ...
    this.sub = this.route
      ...
      .subscribe(params => {
        this.genericClientService.get(uriService)
          .subscribe(result => {this.chequeoMastitisList = result; this._spinnerService.hide(); },
                       err  => this.cathException('error_' + err)
          );
      });
   }
   
   deleteChequeoMastitis(queryParam: Array<[string, string]>) {
   	this._spinnerService.show();
    let idResult: any = 0;
    let serviceProvider : string = 'ChequeoMastitis';
    let pathParams : Array<[number, string]> = null;
  ...
  getConfirmationResponse($event : [boolean, any]) {
    if ($event[0]) {
      ...
      this.onChequeoMastitisDelete($event[1]);
    }
  }
