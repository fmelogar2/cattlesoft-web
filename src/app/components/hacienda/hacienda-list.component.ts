...
import { Hacienda } from '../../core/domain/hacienda.domain';
import { HaciendaMetadata } from '../../core/domain/hacienda.domain';
import { HaciendaDomain } from '../../core/domain/hacienda.domain';
...
@Component({
  selector: 'hacienda-list',
  templateUrl: './hacienda-list.component.html',
  providers:  [UriProvider, GenericClientService]
})
export class HaciendaListComponent implements OnInit {

  private sub: any;
  private haciendaList : Hacienda[];
  private queryParamList: [string, string][] = [['','']];

  private metadata : HaciendaMetadata;
  private haciendaDomain : HaciendaDomain = new HaciendaDomain();
  private fieldList: Field[];
  private modalDeleteMessage : ConfirmationModalOptions;
  private openGenericSearch : boolean = false;
  private localAlert : AlertObject = new AlertObject(null, null, null, 0);
	
  private selectedHacienda: Hacienda;
  constructor(
  ...
  ngOnInit(): void {
  	this.fieldList = this.haciendaDomain.getSearchListOfFields();
    this.metadata = this.haciendaDomain.getHaciendaMetadata();
	this.getHaciendaList(null);
  ...
  advancedSearch() {
   if (this.queryParamList !== null && this.queryParamList.length > 0) {
     if (this.queryParamList.length === 1 && this.queryParamList[0][0].length === 0) {
       ...
       this.getHaciendaList(this.queryParamList);
     }
   	 } else {
       this.getHaciendaList(null);
     }
   ...
   onHaciendaDelete(hacienda : Hacienda) {
    let queryParam : Array<[string, string]> = new Array<[string, string]>();
    queryParam.push(['idHacienda', hacienda.idHacienda.toString()]);
    this.deleteHacienda(queryParam);
  }
  
  onSelectedHacienda(hacienda: Hacienda){
    this.selectedHacienda = hacienda;
  }
   
   getHaciendaList(queryParamList: [string, string][]) {
   	this._spinnerService.show();
    let serviceProvider : string = 'Hacienda';
    ...
    this.sub = this.route
      ...
      .subscribe(params => {
        this.genericClientService.get(uriService)
          .subscribe(result => {this.haciendaList = result; this._spinnerService.hide(); },
                       err  => this.cathException('error_' + err)
          );
      });
   }
   
   deleteHacienda(queryParam: Array<[string, string]>) {
   	this._spinnerService.show();
    let idResult: any = 0;
    let serviceProvider : string = 'Hacienda';
    let pathParams : Array<[number, string]> = null;
